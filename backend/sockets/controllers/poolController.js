...
module.exports = {
    ...
    createPoolForEvent: async (socket, userId, poolData, callback) => {
        try {
            const io = socket.server;
            console.log("the data in the controller", poolData.channelId)
            const event = await Channel.findById(poolData.eventId);
            const eventId = poolData.eventId;
            if (!event.members.includes(socket.user?._id)) {
                throw new Error("Unauthorized: Missing user ID");
            }
            ...
            console.log("the updated channel is", updateEvent)
    ...
    closePoolForEvent: async (socket, userId, poolData, callback) => {
        try {
            console.log("pool data is", poolData);
            const pool = await ChannelPool.findById(poolData.poolId);
            if (!pool.createdBy.equals(userId)) {
    ...
    voteToPoolForEvent: async (socket, userId, poolData, callback) => {
        try {
            const poll = await EventPool.findById(poolData.eventId);
            const io = socket.server;
            const channelId = poolData.channelId;
            console.log("the channel Id is", channelId);
            if (!poll) return callback({ success: false, message: "Poll not found" });
    ...
    getPoolForEvent: async (socket, userId, poolData, callback) => {
        try {
            const channel = await ChannelPool.findById(poolData.channelId);
            if (!channel.members.includes(userId)) {
                callback({ success: false, error: "You are not a member of this channel" });
