...
export const useEventActions = ({
    ...
    const handleAcceptRequest = async (eventId, senderId) => {
        try {
            ...
            // Refresh the events to update the requests list
            fetchMyEvents();
    ...
    const handleRejectRequest = async (eventId, senderId) => {
        try {
            await axios.put(
                'http://localhost:5000/api/events/reject-event-connection-sendby-by-otheruser',
                {
                    eventId: eventId,
                    senderId: senderId
                },
